...
def save_code_and_label(params, path):
    ...
    try:
        fPath = osp.join(config["output_path"], "exported.csv")
        file = open(fPath, "w")
        np.savetxt(file, validation_code, delimiter=',', fmt="%d")
        file.close()
    except IOError:
...
def predict(config):
    ...
    database_codes, database_labels = code_predict(
        dset_loaders, base_network, "database", test_10crop=prep_config["test_10crop"], gpu=use_gpu)
    test_codes, test_labels = code_predict(
        dset_loaders, base_network, "test", test_10crop=prep_config["test_10crop"], gpu=use_gpu)

    return {"database_code": database_codes.detach().cpu().numpy(), "database_labels": database_labels.detach().cpu().numpy(),
            "test_code": test_codes.detach().cpu().numpy(), "test_labels": test_labels.detach().cpu().numpy()}
...
if __name__ == "__main__":
    ...
    config["snapshot_path"] = "../snapshot/"+config["dataset"]+"_" + \
        str(args.hash_bit)+"bit_"+args.prefix + \
        "/"+args.snapshot+"_model.pth.tar"
    config["output_path"] = "../snapshot/"+config["dataset"] + \
        "_"+str(args.hash_bit)+"bit_"+args.prefix
    ...
    print("MAP: " + str(mAP))
    print("saving ...")
    save_code_and_label(code_and_label, osp.join(
        config["output_path"], args.snapshot))
    print("saving done")
