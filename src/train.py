...
def image_classification_predict(loader, model, test_10crop=True, gpu=True, softmax_param=1.0):
    ...
    if test_10crop:
        ...
        for i in range(len(loader['test0'])):
            ...
            if gpu:
                for j in range(10):
                    inputs[j] = Variable(inputs[j].to(mps_device))
                labels = Variable(labels.to(mps_device))
            else:
                for j in range(10):
        ...
        for i in range(len(loader['test'])):
            ...
            inputs = data[0]
            if gpu:
                inputs = Variable(inputs.to(mps_device))
            else:
            ...
            if start_test:
                ...
                all_output = torch.cat(
                    (all_output, outputs.data.cpu().float()), 0)
                all_softmax_output = torch.cat(
                    (all_softmax_output, softmax_outputs.data.cpu().float()), 0)
                all_label = torch.cat((all_label, labels.data.float()), 0)
...
def image_classification_test(loader, model, test_10crop=True, gpu=True):
    ...
    accuracy = torch.sum(torch.squeeze(predict).float() ==
                         all_label) / float(all_label.size()[0])
    return accuracy
...
def train(config):
    ...
    if use_gpu:
        base_network = base_network.to(mps_device)
    ...
    for i in range(config["num_iterations"]):
        if (i % config["snapshot_interval"]) == 0:
            torch.save(nn.Sequential(base_network), os.path.join(config["output_path"],
        ...
        inputs1, labels1 = next(iter1)  # iter1.next()
        inputs2, labels2 = next(iter2)  # iter2.next()
        if use_gpu:
            inputs1, inputs2, labels1, labels2 = Variable(inputs1).to(mps_device), Variable(inputs2).to(mps_device), \
                Variable(labels1).to(mps_device), Variable(
                    labels2).to(mps_device)
        else:
        ...
        similarity_loss = loss.pairwise_loss(outputs.narrow(0, 0, inputs1.size(0)),
                                             outputs.narrow(
                                                 0, inputs1.size(0), inputs2.size(0)),
                                             labels1, labels2,
        ...
        print("Iter: {:05d}, loss: {:.3f}".format(
            i, similarity_loss.float().data))
        config["out_file"].write("Iter: {:05d}, loss: {:.3f} ".format(i,
                                                                      similarity_loss.float().data))
        optimizer.step()

        fPath = osp.join(config["output_path"], "loss.txt")
        lossData = similarity_loss.data.detach().cpu().item()
        try:
            file = open(fPath, "a+")
            file.write(f"{str(lossData)}, ")
        except IOError:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str,
                        default='coco', help="dataset name")
    parser.add_argument('--hash_bit', type=int, default=48,
                        help="number of hash code bits")
    parser.add_argument('--net', type=str, default='ResNet50',
                        help="base network type")
    parser.add_argument('--prefix', type=str, help="save path prefix")
    parser.add_argument('--lr', type=float, help="learning rate")
    parser.add_argument('--class_num', type=float,
                        help="positive negative pairs balance weight")
    args = parser.parse_args()
    os.environ["MPS_DEVICE"] = args.gpu_id
    ...
    if "ResNet" in args.net:
        ...
        config["network"]["params"] = {
            "name": args.net, "hash_bit": config["hash_bit"]}
    ...
    config["loss"] = {"l_weight": 1.0, "q_weight": 0, "l_threshold": 15.0,
                      "sigmoid_param": 10./config["hash_bit"], "class_num": args.class_num}

    if config["dataset"] == "imagenet":
        config["data"] = {"train_set1": {"list_path": "../data/imagenet/train.txt", "batch_size": 18},
                          "train_set2": {"list_path": "../data/imagenet/train.txt", "batch_size": 18}}
    elif config["dataset"] == "nus_wide":
        config["data"] = {"train_set1": {"list_path": "../data/nus_wide/train.txt", "batch_size": 18},
                          "train_set2": {"list_path": "../data/nus_wide/train.txt", "batch_size": 18}}
    elif config["dataset"] == "coco":
        config["data"] = {"train_set1": {"list_path": "../data/coco/train.txt", "batch_size": 18},
                          "train_set2": {"list_path": "../data/coco/train.txt", "batch_size": 18}}
    print(config["loss"])
    train(config)
